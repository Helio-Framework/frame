...
public class VelocityEvaluator {
	...
	static {
		...
		functionClasses.put("HF", VelocityFunctions.class);
	}
	
	public static void addFunctionClass(String name, Class<?> clazz) {
		functionClasses.put("HF", clazz);
	}
	...
	public static StringWriter evaluateTemplate(String templateId, Map<String, List<String>> dataReferences) {
		...
		// Load functions
		functionClasses.entrySet().parallelStream().forEach(entry -> context.put(entry.getKey(), entry.getValue()));
		// Load references
	...
	public static StringWriter evaluateTemplate(String templateId, String jsonPayload) {
		...
		Map<String, List<String>> simplifiedMatrix = FrameworkUtils.GSON.fromJson(jsonPayload, new HashMap<String, List<String>>().getClass());
